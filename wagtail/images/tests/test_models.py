...
class FakeImage:
    def create_renditions(self, *filters):
        if not filters:
            return {}
        if len(filters) == 1:
            filter = filters[0]
            fake_rendition = Mock()
            fake_rendition.filter_spec = filter.spec
            return {filter: fake_rendition}
...
class TestRenditions(TestCase):
    ...
    def test_create_renditions_mcdc_ct1(self):
        result = self.image.create_renditions()
        self.assertEqual(result, {})

    def test_create_renditions_mcdc_ct3(self):
        filter_obj = Filter("width-100")
        result = self.image.create_renditions(filter_obj)
        self.assertEqual(len(result), 1)
        self.assertIn(filter_obj, result)
        self.assertEqual(result[filter_obj].filter_spec, filter_obj.spec)
